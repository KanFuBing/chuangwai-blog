...
import { addFbDoc, authentication } from '../utils/firebase'
    ...
    const send = () => {
        if (user) {
            ...
            addFbDoc('comments', { user: user.displayName, uid: user.uid, time: serverTimestamp(), md: value, article: articleId })
                .then(docRef => {
                    setValue('')
                    setUserComments(comments => [...comments, { id: docRef.id, user: user.displayName as string, uid: user.uid, time: { seconds: Date.now() / 1000 }, md: value, article: articleId }])
                    setDisabled(false)
                    setOpen(true)
