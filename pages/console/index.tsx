...
import { addFbDoc, db, storage } from '../../utils/firebase'
          ...
          <nav key={article.id}>
            <ListItem>
              <ListItemButton onClick={() => router.push(`/console/article/${article.id}`)}>
                <ListItemText primary={`${article.title} （时间：${new Date(article.time.seconds * 1000).toLocaleDateString()}，ID：${article.id}）`} />
              </ListItemButton>
            </ListItem>
            <Divider />
          </nav>
...
export const getServerSideProps: GetServerSideProps = async ctx => {
  const articlesSnapPromise = getDocs(query(collection(db, 'articles'), orderBy('time', 'desc')))
